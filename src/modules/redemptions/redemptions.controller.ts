...
export class RedemptionController {
  ...
  @ApiResponse({ status: 201, description: 'Redemption processed successfully', type: RedemptionResponseDto })
  async processRedemption(@Body() createRedemptionDto: CreateRedemptionDto): Promise<RedemptionResponseDto> {
    return this.redemptionService.processRedemption(
      createRedemptionDto.qrToken,
      createRedemptionDto.staffId,
  ...
  @ApiResponse({ status: 200, description: 'Redemption validation result', type: RedemptionValidationDto })
  async validateRedemption(
    @Body() body: { qrToken: string; staffId: string },
  ...
  @ApiQuery({ name: 'status', required: false, enum: RedemptionStatus, description: 'Filter by redemption status' })
  ...
  @ApiResponse({ status: 200, description: 'Redemption statistics retrieved successfully', type: RedemptionStatsDto })
  @ApiQuery({ name: 'merchantId', required: false, type: String, description: 'Filter by merchant ID' })
  async getRedemptionStats(@Query('merchantId') merchantId?: string): Promise<RedemptionStatsDto> {
  ...
  @ApiResponse({ status: 200, description: 'Redemption analytics retrieved successfully', type: RedemptionAnalyticsDto })
  @ApiQuery({ name: 'merchantId', required: false, type: String, description: 'Filter by merchant ID' })
  @ApiQuery({ name: 'startDate', required: false, type: Date, description: 'Filter by start date' })
  @ApiQuery({ name: 'endDate', required: false, type: Date, description: 'Filter by end date' })
  ...
  @ApiResponse({ status: 200, description: 'Redemption retrieved successfully', type: RedemptionResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Redemption status updated successfully', type: RedemptionResponseDto })
  async updateStatus(
    @Param('id') id: string,
  ...
  @Roles(UserRole.ADMIN, UserRole.MERCHANT, UserRole.STAFF)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get redemptions by staff member' })
  @ApiResponse({ status: 200, description: 'Staff redemptions retrieved successfully' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page' })
  @ApiQuery({ name: 'status', required: false, enum: RedemptionStatus, description: 'Filter by redemption status' })
  ...
  @ApiResponse({ status: 200, description: 'Merchant redemptions retrieved successfully' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page' })
  @ApiQuery({ name: 'status', required: false, enum: RedemptionStatus, description: 'Filter by redemption status' })
  @ApiQuery({ name: 'startDate', required: false, type: Date, description: 'Filter by start date' })
  ...
  @ApiResponse({ status: 200, description: 'Customer redemptions retrieved successfully' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page' })
  @ApiQuery({ name: 'status', required: false, enum: RedemptionStatus, description: 'Filter by redemption status' })
